...
- `databricks_sql_pattern_index.json` -> `chris_demos.demos.databricks_sql_pattern_index`
- `databricks_sql_pattern_registry.json` -> `chris_demos.demos.databricks_sql_pattern_registry`
- `databricks_sql_pattern_composition_rules.json` -> `chris_demos.demos.databricks_sql_pattern_composition_rules`

`project_manifest.json` is included for completeness but is not required as a Databricks table by the skill.

## Raw pattern packs

The directory `databricks_sql_pattern_packs/` contains the full raw Databricks SQL pattern library delivered as 15 iteration files.

Those files are runtime assets and should stay on disk exactly as provided. They are used for:
- deeper pattern examples
- original pack context
- disk-based fallback when pattern support tables are missing

They are **not** loaded into separate Databricks tables by default.

## Notes

These support files now encode:
- the multi-pipeline topology (commerce_foundation, journey_diagnostics, product_value_diagnostics, observatory_reporting)
- the datatype contracts in both grouped and flat form
- the global Databricks SQL pattern library, including positive reusable templates and negative guidance entries
