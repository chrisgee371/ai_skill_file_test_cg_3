...
Build only the source-entry layer and verify that the pipeline matches the supplied Databricks source tables exactly.
...
Build the clean internal layer that later stages can trust and consume using the approved dependency patterns: self-contained `{{ source() }}` where feasible, or `{{ ref('model_name') }}` plus matching pipeline connections when reusing model output.
