...
Use stage-2 internal models through the approved patterns:

- Prefer self-contained models driven from `{{ source() }}` where that keeps the logic stable.
- If a diagnostic model must consume a stage-2 model output, use `{{ ref('model_name') }}` and ensure the matching `input_ports` and `>>` connections exist.
