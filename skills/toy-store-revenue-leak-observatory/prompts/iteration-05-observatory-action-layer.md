...
Use stage-3 and stage-4 diagnostic models through the approved patterns:

- Prefer self-contained models driven from `{{ source() }}` where that keeps the logic stable.
- If the observatory layer must consume another model's output directly, use `{{ ref('model_name') }}` together with matching `input_ports` and `>>` connections.
